...
from saim.strain_matching.private.container import DepMatCon, DepositMatch
...
def _verify_ccno_match(mat: DepMatCon[DepositMatch]) -> UpdateResults:
    assert mat.strain_id == 1
    assert mat.deposit_id == 1
    return UpdateResults(si_id=mat.strain_id, si_dp=mat.deposit_id)


def _verify_strain_match(mat: DepMatCon[DepositMatch]) -> UpdateResults:
    assert mat.strain_id == 1
    assert mat.deposit_id == -1
    return UpdateResults(si_id=mat.strain_id, si_dp=mat.deposit_id)


def _verify_strain_si_id_match(mat: DepMatCon[DepositMatch]) -> UpdateResults:
    assert mat.strain_id == 2
    assert mat.deposit_id == -1
    return UpdateResults(si_id=mat.strain_id, si_dp=mat.deposit_id)


def _verify_mis_match(mat: DepMatCon[DepositMatch]) -> UpdateResults:
    assert mat.strain_id == -1
    assert mat.deposit_id == -1
    return UpdateResults(si_id=mat.strain_id, si_dp=mat.deposit_id)
...
def test_strain_rel_si_id_match(
    acronym_manager: AcronymManager,
    cache_strain_match: MatchCache,
    ccno_dsmz_si_id_re1: DepositMatch,
) -> None:
...
def test_strain_rel_ccno_match(
    acronym_manager: AcronymManager,
    cache_strain_match: MatchCache,
    ccno_dsmz_ccno_re1: DepositMatch,
) -> None:
