...
def _extract_date(pos_date: str, cur_year: int, /) -> str | None:
    if (
        ...
        and _is_reasonable_sampling_date(pos_date, int(year.group(1)), cur_year)
    ):
        return par_date
    return None
...
def _is_reasonable_sampling_date(date: str, ext_year: int, cur_year: int, /) -> bool:
...
def get_date(pos_date: str, /) -> CompleteDate | None:
    ...
    try:
        ...
        if _is_reasonable_sampling_date(pos_date, parsed_date.year, cur_year):
